...
public class ImpTeacher {
	...
	public HashMap<String, Object> save(File file) {
		...
		// 教师集合
		ArrayList<Record> teachers = new ArrayList<Record>();
		// 错误信息集合
		...
		for (int sheetNum = 0; sheetNum < workbook.getNumberOfSheets(); sheetNum++) {
			...
			for (int rowNum = 1; rowNum <= lastRowNum; rowNum++) {
				...
				// 校验是否存在此教师
				Record teachersById = Db.findFirst("select * from tb_teachers where id = ?", id);
				if (teachersById != null) {
					rowInfo += "工号为" + id + "的老师已录入；";
					hasError = true;
				}
				...
				if (hasError) {// 单行有错误
					...
					teachers.add(new Record().set("id", id).set("name", name).set("sex", sex).set("faculty", faculty).set("password", password));
				}
			}
		}
		// 判断数据是否有错误
		if (errs.isEmpty()) {// 数据无错误
			String sql = "INSERT INTO tb_teachers(id,name, sex, birthday,national,phone,faculty,title,office,password) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?,?)";
			Db.batch(sql, "id,name, sex, birthday,national,phone,faculty,title,office,password", teachers, 5000);
			res.put("status", "ok");
			res.put("msg", "导入成功，增加" + teachers.size() + "条记录！");
		} else {// 数据有错误
			res.put("status", "fail");
			res.put("msg", errs);
		}
